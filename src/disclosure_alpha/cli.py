...
def cmd_companies(args):
    ...
    settings = load_settings()
    df = build_universe(settings, EDisclosureClient(settings), limit=args.limit, resolve_edisclosure_ids=not args.no_resolve)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s = sub.add_parser("discover", help="скачать ключевые страницы e-disclosure и описать их структуру")
    s.add_argument("--out")
    s.set_defaults(func=cmd_discover)

    s = sub.add_parser("companies", help="справочник акций MOEX (ISS) + сопоставление с компаниями e-disclosure по ИНН")
    ...
    s = sub.add_parser("messages", help="собрать строки поиска сообщений за период")
    ...
    s = sub.add_parser("events", help="загрузить тексты сообщений и разобрать сделки инсайдеров; собрать таблицу событий")
