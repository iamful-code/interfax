...
* **Капча.** Сайт иногда показывает капчу с поворотом картинки. Её проходит человек, один раз:
  ...
  Пройденная капча сохранится в профиле `data\browser_profile\`, поэтому следующие команды, в том числе
  без видимого окна, пойдут уже без неё. Если окно скрыто, а сайт показал капчу, команда не виснет:
  она сообщает, что нужно перезапустить с `--show-browser`.
