...
* **Капча.** Сайт иногда показывает капчу с поворотом картинки. Её проходит человек, один раз:
  ```powershell
  disclosure-alpha discover --show-browser
  ```
  Откроется окно браузера, пройдите капчу в нём — команда сама продолжит работу (ждёт до 5 минут).
  Если капча появилась при запуске без `--show-browser`, команда сама откроет окно и подождёт, пока
  вы её пройдёте: перезапускать не нужно. Отключить это поведение: `$env:DA_BROWSER_AUTO_VISIBLE = "0"`
  (тогда команда просто сообщит, что нужен `--show-browser`).
  Пройденная капча сохраняется в профиле `data\browser_profile\`, но защита может запросить её снова,
  особенно после паузы. Если капча появляется часто, работайте с `--show-browser`.
* Окно не нужно держать открытым: команда закрывает браузер по завершении.
* Если содержимое страницы рисуется скриптом, команда дожидается затишья в сети и небольшой паузы
  (`$env:DA_BROWSER_SETTLE_MS`, по умолчанию 1500 мс) и только потом читает страницу.
